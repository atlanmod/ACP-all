...
#         chest_pain_locn(X, center_of_chest), chest_pain_locn(X, right_side), chest_pain_locn(X, left_side), # base
#     childhood_rfever(X), chronic_penicil_usr(X), fever_with_jntpain(X), sore_throat_fever(X), # rfever
#       cp_radiates(X),  (age(X) >= 10), (age(X) <= 40), 
#       (age(X) < 40), is_nails_blue(X), (age(X) < 15),
#       val_surgery(X), # suspected_disease
#         bp_status(X), (systolic_bp(X) >= 160), (diastolic_bp(X) >= 95), (number_of_cigs(X) > 0), (number_of_cigs(X) == 0), 
#         (number_of_drinks(X) == 0), (number_of_drinks(X) > 0), # other
#         sedentary(X), managerial(X), ambitious(X), (age(X) >= 55), 
#         (age(X) >= 30), (bmi(X) > 24), # IH_risk
#         filariasis(X), complaint(X, syncope), epileptic(X),
#         dysp_on_exertion(X), dysp_at_rest(X), lung_disease(X), palpitations(X), complaint(X, swelling_of_feet), 
#         (age(X) > 15), (age(X) < 45), pregnant(X), kidney_disease(X), face_swelling(X), orthopnoea(X),
#         PND(X),  # HD
#         (age(X) > 40), bypass_surg(X), 
#                chorea(X), (hb_level(X) > 15), clubbed_fingers(X), # disease
#                complaint(X, chest_pain), 
#             cough_linked_cp(X), sweating_linked_cp(X), exertion_linked_cp(X), 
#             # chest_pain
#             complaint(X, tiredness),  # card_symp
#             (bmi(X) == 30), (bmi(X) == 24), (bmi(X) <= 30), (number_of_cigs(X) == 5), (number_of_cigs(X) > 5), (number_of_cigs(X) <= 10), (number_of_cigs(X) > 10),
#             # adv
#                   ]
